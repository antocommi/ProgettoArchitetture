...
double* get_blockD(int size, int elements) {
	double* punt= _mm_malloc(elements*size,32);
        return punt;
}
...
MATRIX load_dense(char* filename, int *n, int *m, int *nr, int *mr) {
	...
	status = fread(&cols, sizeof(int), 1, fp);
        restoN = 32-cols%32;
        if(restoN==32) restoN=0;
        restoM = 64-cols%64;
        if(restoM==64) restoM=0;
...
csc creaGF(location* L, double c, int N, int M, int restoM){
	...
	df = (float *) _mm_malloc ((N+restoM)*sizeof(float),32);
	col = (int *) _mm_malloc ((N+1)*sizeof(int),32);
	row = (int *) _mm_malloc (M*sizeof(int),32);
	for(i=0;i<N;i++){
	   col[i]=0;
	   df[i]=0.0;
...
csc creaGD(location* L, double c, int N, int M, int restoM){
	...
	dd = (double *) _mm_malloc ((N+restoM)*sizeof(double),32);
	col = (int *) _mm_malloc ((N+1)*sizeof(int),32);
	row = (int *) _mm_malloc (M*sizeof(int),32);
	for(i=0;i<N;i++){
	   col[i]=0;
...
GRAPH load_sparse(char* filename, int *n, int *m,double c,int prec, int* rN, int* rM) {
	...
	if(prec==SINGLE){
		restoN= 64-nodes%64;
		if(restoN==64)restoN=0;
		restoM= 128-nodes%128;
		if(restoM==128) restoM=0;
	}
	else{
		restoN= 32-nodes%32;
		if(restoN==32)restoN=0;
    ...
    g =(location *) _mm_malloc(arcs*sizeof(location),32);
	for (i = 0; i < arcs; i++) {
...
void norma1d( VECTORD v, int dim, double* norma) {
	int i;
	double norm=0.0;
...
void norma1f( VECTORF v, int dim, float* d) {
    int i;
    float norm=0.0;
...
extern void differenceNormd(VECTORD v1, VECTORD v2,int dim,double* delta, int dim2);
...
extern void differenceNormf(VECTORF v1, VECTORF v2,int dim, float* delta,int dim2);
...
void pagerankfullNoOpt(params* input){
	int i;
	double delta;
	double* temp = _mm_malloc(sizeof(double)*(input->N+input->restoM),32);
    for(i=0;i<input->N;i++){
	...
	while(delta>=input->eps){
		prod(input->P,input->pagerank,input->N+input->restoN,input->N,temp);
	        differenceNormd(input->pagerank,temp,input->N+input->restoM,&delta,input->N);
                assegnaD(input->pagerank,temp,input->N+input->restoM);
	}
      normalizzazione(input->pagerank,input->N,input->N+input->restoM);
      _mm_free(temp);
      _mm_free(input->P);
...
void pageranksparseNoOptFloat(params* input){
    int i,j;
    float delta,s;
    float* xk= _mm_malloc(sizeof(float)*(input->N+input->restoM),32);
    float* xk1= _mm_malloc(sizeof(float)*(input->N+input->restoM),32);
    float* v= _mm_malloc(sizeof(float)*(input->N+input->restoM),32);
    ...
    while(delta>=input->eps){
        ...
        differenceNormf(xk1,xk,input->N+input->restoM,&delta,input->N);
...
void pageranksparseNoOptDouble(params* input){
    int i,j;
    double delta,somma;
    double* xk= _mm_malloc(sizeof(double)*(input->N+input->restoM),32);
    double* v= _mm_malloc(sizeof(double)*(input->N+input->restoM),32);
    for(i=0;i<input->N;i++){
        input->pagerank[i] = 1.0/input->N;
        xk[i]=0.0;
        v[i]=0.0;
    ...
    while(delta>=input->eps){
          ...
          differenceNormd(input->pagerank,xk,input->N+input->restoM,&delta,input->N);
...
void pagerankfullOpt(params* input){
    ...
    double delta,p;
    double* temp = _mm_malloc(sizeof(double)*(input->N+input->restoM),32);
    double* l = _mm_malloc(sizeof(double)*(input->N+input->restoM),32);
...
void pageranksparseOptFloat(params* input){
    ...
    float* xk = _mm_malloc(sizeof(float)*(input->N+input->restoM),32);
    float* xk1 = _mm_malloc(sizeof(float)*(input->N+input->restoM),32);
    float* l = _mm_malloc(sizeof(float)*(input->N+input->restoM),32);
    float* v= _mm_malloc(sizeof(float)*(input->N+input->restoM),32);
...
void pagerank(params* input) {
    int i;
    input->pagerank=(double*) _mm_malloc((input->N+input->restoM)*sizeof(double),32);
