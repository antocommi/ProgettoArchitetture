...
float dist_simmetrica(params* input, int centroide1, int centroide2){
	...
	float pow=0;
	for(i=0; i<input->m; i++){
	pow=((dist_simmetricaI(input, centroide1, centroide2, i*input->m, (i+1)*input->m)*(dist_simmetricaI(input, centroide1, centroide2, i*input->m, (i+1)*input->m));
		sum+=pow;
	}
	return sum;
}
...
float dist_asimmetricaI(params* input, MATRIX set, int punto, int centroide2, int start, int end){
	...
	float pow=0;
	for(i=start; i<end; i++){
		pow=((set[punto*input->d+i] - input->codebook[centroide2*input->d+i])*( set[punto*input->d+i] - input->codebook[centroide2*input->d+i]));
		ret += pow;
	}
	return ret;
}
...
float dist_asimmetrica(params* input, MATRIX set, int punto, int centroide){
	...
	float pow=0;
	for(i=0; i<input->m; i++){
		pow=((dist_asimmetricaI(input, set, punto, input->pq[centroide], i*input->m, (i+1)*input->m)*(dist_asimmetricaI(input, set, punto, input->pq[centroide], i*input->m, (i+1)*input->m));
		sum+=pow;
...
float dist(params* input, int* quantizer, int punto, int centroide){
	...
	float pow=0;
	for(i=0; i<input->m; i++){
		pow=((distI(input, quantizer, punto, input->pq[centroide], i*input->m, (i+1)*input->m)*(distI(input, quantizer, punto, input->pq[centroide], i*input->m, (i+1)*input->m));
		sum+=pow;
	}
	return sum;
}
...
void kmeans_from(params* input, struct kmeans_data* data, int start, int end ){
	...
	for(t=0; t<input->tmin || (t<input->tmax && (fob2-fob1) > input->eps); t++){
		...
		for(i=0; i<input->nr; i++){
			pow=((dist_eI(input, data, i, data->index[i*data->index_colums+start/dStar], start, end)*(dist_eI(input, data, i, data->index[i*data->index_colums+start/dStar], start, end));
			fob2+=pow;
