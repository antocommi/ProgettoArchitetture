...
float dist_asimmetrica(params* input, MATRIX set, int punto1, int punto2){
	...
	for(i=0; i<input->m; i++){
		...
		dist_eI(ind1, ind2+(*c2)*input->d, dStar, &temp);
		sum+=pow2(temp, 2);
		ind1+=dStar;
		ind2+=dStar;
		c2++;
...
int calcolaQueryPQ(params* input, int start, int end){
	// estremi start incluso ed end escluso
    //
    ...
    float min;
    float temp;
	int* ind=input->query_pq+start/(input->d/input->m);
	float *ind1, *ind2;
	//printf("breakpoint PQ\n");
	ind1=input->qs+start;
	for(j=0; j<input->nq; j++){
		min=1.79E+308;
		ind2=input->codebook+start;
		for(i=0; i<input->k; i++){
			//dist_asimmetricaI(input, input->qs, j, i, start, end, &temp);
			dist_eI(ind1, ind2, end-start, &temp);
			if(temp<min){ 
				min=temp;
				*ind=i;
			}
...
void calcolaPQ(params* input, int start, int end){
	int i, j;
	int m=input->m;
	float min;
	float temp;
	float *ind1, *ind2;
	int* ind=input->pq+start/(input->d/input->m);
	//printf("before for\n");
	ind1=input->ds+start;
	for(i=0; i<input->n; i++){
		min=1.79E+308;
...
//-----------------------------------
		...
		for(i=0; i<input->n; i++){
			...
			dist_eI(ind+input->pq[i*m+ipart]*input->d, ind2, end-start, &temp);
			//printf("%f\n", temp);
			fob2+=pow2(temp, 2.0);
			//printf("%f\n", fob2);
			ind2+=input->d;
